...
const searchRecipes = (event) => {
  ...
  const searchResults = displaySearchRecipes(
    searchTextUser,
    displaySearchRecipesWithTags(ingredientsTags, ustensilesTags, appareilsTags, recipesData),
  );
...
const resetForm = () => {
  ...
  recipeSection.textContent = '';
  searchTextUser = '';
  const result = displaySearchRecipes(
    searchTextUser,
    displaySearchRecipesWithTags(ingredientsTags, ustensilesTags, appareilsTags, recipesData),
  );
...
const displayTags = (list) => {
  ...
  const ingredientsSearchTags = filterTagsSearch(
    ingredientsInput.value.toLowerCase(),
    ingredientsArray,
  );
  const appareilsSearchTags = filterTagsSearch(appareilsInput.value.toLowerCase(), appareilsArray);
  const ustensilesSearchTags = filterTagsSearch(
    ustensilesInput.value.toLowerCase(),
    ustensilesArray,
  );

  // Récupération des tags des recettes actuellement affichées après la recherche
  const displayedRecipes = displaySearchRecipes(
    searchTextUser,
    displaySearchRecipesWithTags(ingredientsTags, ustensilesTags, appareilsTags, recipesData),
  );
...
const addActiveTag = (tag, tagName, typeTag) => {
  ...
  const allTags = displaySearchRecipes(
    searchTextUser,
    displaySearchRecipesWithTags(ingredientsTags, ustensilesTags, appareilsTags, recipesData),
  );
